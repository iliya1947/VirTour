  ...
  return (
    <main lang={locale} dir={isRtl(locale) ? 'rtl' : 'ltr'} className="h-[calc(100vh-1rem)] overflow-hidden bg-slate-950 p-2 md:p-4">
      <section className="relative mx-auto grid h-full w-full max-w-[1780px] grid-cols-1 overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/95 shadow-[0_24px_80px_rgba(2,6,23,.45)] md:grid-cols-[minmax(440px,42vw)_1fr]">
              ...
              {filtered.map((item) => {
                ...
                return (
                  <article
                    ...
                    className={`group cursor-pointer overflow-hidden rounded-2xl border transition-all ${active ? 'border-cyan-300/80 bg-cyan-300/12 shadow-[0_10px_36px_rgba(34,211,238,.2)]' : 'border-white/10 bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.07]'}`}
                  >
        ...
        <div className={`relative min-h-0 ${mobileTab === 'list' ? 'hidden md:block' : 'block'}`}>
          ...
          {filtered.map((item) => {
            ...
            return (
              <button
                ...
                onMouseLeave={() => setHoverSlug('')}
                onClick={() => setActiveSlug(item.slug)}
                className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full border px-3.5 py-1.5 text-xs font-semibold shadow-xl transition-all duration-200 ${active ? 'z-30 scale-110 border-cyan-100 bg-cyan-300 text-slate-950' : 'z-20 border-white/35 bg-black/55 text-white backdrop-blur-xl hover:scale-105 hover:bg-black/75'}`}
              >
          ...
          {activeItem && (
            <div className="absolute bottom-4 end-4 z-40 w-[92%] max-w-sm rounded-2xl border border-white/20 bg-slate-900/85 p-3 shadow-2xl backdrop-blur-2xl">
              <div className="relative h-32 overflow-hidden rounded-xl">
                <Image src={activeItem.images[0]} alt={activeItem.title} fill className="object-cover" />
              </div>
